...
- [ ] Start the shared local services once per host with `docker compose up -d postgres minio` from the repo root (PR1 adds both services bound to `127.0.0.1`). Create the lane database with `psql "postgresql://shadowlearn:shadowlearn@127.0.0.1:5432/postgres" -c 'create database lane_<n>'`.
- [ ] Start the backend with `cd backend && SHADOWLEARN_DATABASE_URL="postgresql+asyncpg://shadowlearn:shadowlearn@127.0.0.1:5432/lane_<n>" SHADOWLEARN_S3_BUCKET="lane-<n>" SHADOWLEARN_FRONTEND_ORIGIN_ALLOWLIST='["http://127.0.0.1:52<nn>"]' uv run alembic upgrade head && uv run uvicorn app.main:app --port 81<nn>`. Wait for `curl -sf http://127.0.0.1:81<nn>/api/health/deps` to return `{"db":"ok","s3":"ok"}`. At trunk, or at a head below PR1, start the backend the old way with `uv run uvicorn app.main:app --port 81<nn>` and its `.env`.
...
- [ ] Add to `Settings` in `backend/app/settings.py` the fields `database_url` (default `postgresql+asyncpg://shadowlearn:shadowlearn@127.0.0.1:5432/shadowlearn`), `s3_endpoint` (default `http://127.0.0.1:9000`), `s3_access_key`, `s3_secret_key`, `s3_bucket` (default `shadowlearn`), `s3_region` (default `us-east-1`), and `temp_dir` (default `/tmp/shadowlearn`).
...
- [ ] In both compose files add `postgres` (`postgres:16-alpine`, volume `pgdata`, port `127.0.0.1:5432:5432`, healthcheck `pg_isready -U shadowlearn`) and `minio` (`minio/minio`, command `server /data`, volume `miniodata`, port `127.0.0.1:9000:9000`, healthcheck on `/minio/health/live`). The `backend` service depends on both with `condition: service_healthy` and its command becomes `sh -c "alembic upgrade head && uvicorn app.main:app --host 0.0.0.0 --port 8000"`.
- [ ] Add `db_session` and `s3` fixtures to `backend/tests/conftest.py`. `db_session` runs `alembic upgrade head` once per session against `SHADOWLEARN_TEST_DATABASE_URL` (default `postgresql+asyncpg://shadowlearn:shadowlearn@127.0.0.1:5432/shadowlearn_test`) and truncates every table after each test. `s3` creates bucket `shadowlearn-test-<pid>` for the session and removes it at the end.
- [ ] Create `.github/workflows/backend-tests.yml` with a `postgres:16-alpine` service, a step that runs `docker run -d -p 9000:9000 -e MINIO_ROOT_USER -e MINIO_ROOT_PASSWORD minio/minio server /data` and waits for `/minio/health/live` (GitHub service containers cannot set a command), then `uv sync --extra dev` and `uv run pytest`. Trigger on `pull_request` and `push` to `main`.
