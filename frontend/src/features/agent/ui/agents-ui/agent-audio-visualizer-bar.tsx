...
import { useAgentAudioVisualizerBarAnimator } from '@/features/agent/application/agents-ui/use-agent-audio-visualizer-bar'
...
export function AgentAudioVisualizerBar({
  ...
  const bands = useMemo(
    () => (state === 'speaking' ? volumeBands : Array.from<number>({ length: _barCount }).fill(0)),
