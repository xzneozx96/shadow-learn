...
import type { AgentTool, ToolContext } from '@/features/agent/lib/tools/types'
...
export function truncateIfOversized(result: string, tool: AgentTool): string {
  ...
  if (result.length <= threshold)
    return result
  // Trim at the last newline within the first 10000 chars to avoid mid-line cuts
  const raw = result.slice(0, 10_000)
  const preview = raw.includes('\n') ? raw.split('\n').slice(0, -1).join('\n') : raw
  return (
    `[Result truncated: ${result.length} chars exceeded limit of ${threshold}. `
    + `Showing first 10000 chars]\n\n${preview}\n...`
  )
}
