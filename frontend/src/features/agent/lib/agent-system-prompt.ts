...
function buildDynamicSections(context: SessionContext): string {
  ...
  sections.push(
    ...
    '- **For ANY Chinese grammar question or struggle (a grammar rule, particle, sentence pattern, e.g. the 把 construction, 了/着/过, complements), ALWAYS call `search_document` first with a concise natural-language query. Ground your answer in the returned passages and cite the source — never answer grammar from your own training data, and do NOT use `get_skill_guide` for grammar. When you answer a grammar question from `search_document`, ALWAYS finish by giving the user the YouTube link(s) of the lesson(s) you used so they can watch the source. Only use links found verbatim in the passages — never invent or guess a URL; if a link is broken across lines, rejoin it into a single `https://youtu.be/<id>` URL.**',
